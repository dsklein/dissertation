...
I will focus on the 8 TeV results, and in particular, my work with the
...
In general, a particle's lifetime -- how long it tends to exist before
decaying to lighter particles -- is inversely related to its mass \cite{griffiths}.
The top quark, being so heavy, has an extremely short lifetime
...
normally occur \cite{pdg,spindecorrelation}, processes that
wipe out some information about a quark. Since the top quark decays
before these processes can happen, it passes on information about its spin
...
unexplored territory. Because the likelihood of a decay is
...
properties to deviate from the values predicted by the SM. Thus,
precise measurements of properties such as charge
asymmetry may tease out links to possible new physics.
...
would have to work with. However, the CMS detector was designed
with high-precision lepton reconstruction in mind, and precision is
important to these asymmetry measurements. So we choose to study the two-lepton
...
\caption[Feynman diagram showing a proton-proton collision that
  ...
  leptons ($\ell$) are not necessarily of the same flavor. This
...
of forward ones, and dividing by the total:
...
\emph{absolute value} of rapidity. Thus we define our \textbf{top charge
asymmetry} as:
\begin{equation}
...
need to reconstruct the $\ttbar$ system (a process described in Section
...
For this analysis, we use 19.5 fb\textsuperscript{-1} of data from
the 8 TeV portion of CMS Run I, encompassing the 2012A through 2012D
eras. Because the analysis focuses on a process that produces two
leptons in the final state, we use the dilepton datasets
A complete list of these datasets is given in Table \ref{tab:afb:datasamples}.

%% Table of data samples. Taken from AN-14-246. Believed unpublished so far %%%%%%%
\begin{table}[!ht]
\begin{center}
\caption{List of CMS datasets used in this analysis.}
...
We use several Monte Carlo datasets to model both the $\ttdilep$
...
\begin{table}[pht]
\begin{center}
\caption{List of CMS Monte Carlo samples used in this analysis.}
...
To select events from the datasets listed in Table
\ref{tab:afb:datasamples}, we use a set of dilepton triggers that
capture the four possible dilepton flavors. These triggers are
listed in Table \ref{tab:afb:triggers}.

%% Table of triggers used. Taken from AN-14-246. Believed unpublished so far %%%%%%%
\begin{table}[!ht]
\begin{center}
\caption{List of triggers used in this analysis.}
...
We employ a set of physics object and event selections designed to
...
\item Electrons and muons are defined using the recommended
  ...
  POGs. Specifically, electrons use the recommended medium working
  point, and muons use the recommended tight WP.
...
> 0.4$. B-tagging of jets is done using the CSV
medium working point, as provided by the b-tag POG.
$\metvec$ is computed by summing up the vector
momenta of all particle flow objects, and multiplying by $-1$.
...
key to this method is the fact that the process $Z \rightarrow \ell\ell$
...
and selections from the SR. We form three broad categories of CRs,
each targeting a particular background component, as
...
\begin{figure}[t]
  ...
  \caption{SFs measured in different CRs. The blue line depicts the
    central value, and the red lines give the systematic uncertainty band.}
...
\hline
              ...
              $\ttbar \rightarrow \ell +$ jets   &     107.1 $\pm$ 7.7   &      62.2 $\pm$ 5.4   &    327.4 $\pm$ 13.4   &    496.7 $\pm$ 16.4  \\
                ...
                single top (s/t-chan, 1$\ell$)   &       2.6 $\pm$ 0.6   &       4.6 $\pm$ 0.9   &      18.8 $\pm$ 1.6   &      26.1 $\pm$ 1.9  \\
                    single top ($tW$, 2$\ell$)   &     298.0 $\pm$ 1.6   &     425.9 $\pm$ 1.9   &    1161.9 $\pm$ 3.1   &    1885.8 $\pm$ 4.0  \\
...
The CMS detector and reconstruction software described in Section
\ref{sec:cms} are not perfect. Neither are our object
...
asymmetry values may be distorted from their true values. In addition,
The unique attributes of other experiments may cause their measured
asymmetry values to differ from those at CMS. And finally,
...
disciplines. General unfolding for particle physics is described in detail in
...
response matrix into two components, giving us the matrix equation:
\begin{equation}
\vec{b} = S A \vec{x}
...
\ref{eq:afb:convolution} to solve for the true distribution, $\vec{x}$,
of our variables:
\begin{equation}
\vec{x} = A^{-1} S^{-1} \vec{b}
...
some degree of bias into our measurements. So, in Section
...
artificially well-posed \cite{blobelseminar}. We choose to bin our
variables so that the number of events in each bin is
...
For each asymmetry variable, we calculate the fraction of true
Monte Carlo events in each bin that pass our selections. These
values then become the diagonal entries of the acceptance matrix,
$A$. Because our event selections are slightly different for
same-flavor and opposite-flavor dileptons, we calculate and apply
...
The smearing matrices $S$ are generated based on Monte Carlo
simulations. Each simulated event is placed into a 2D histogram with the
...
converts the histogram bins into elements of the smearing
...
\cite{blobelseminar}. The values of $\tau$ we arrive at this way
are presented in Table \ref{tab:afb:tau1d}. To validate these values,
we check how the unfolded asymmetries and their uncertainties vary as we scale
$\tau$ up and down by up to a factor of 100. As expected, the
uncertainties become larger as $\tau$ is decreased; as $\tau$ is
increased, the asymmetry central value shifts, indicating the addition of bias. From
this behavior, we deduce that our choices for $\tau$ are
...
We perform the actual regularized unfolding using the TUnfold
...
regularize in the secondary variable. In the unwrapped
distribution, this means we must regularize bins that are no longer
...
function of the measured variable. For example, the top charge
asymmetry is defined in terms of the measured variable $|y_t| -
|y_{\bar{t}}|$ (see Equation \ref{eq:afb:topchargeasym}), so the reweighting
equation would be:
\begin{equation}
...
For each $K$ value, we make 10,000 copies of the measurement
...
\emph{pseudoexperiment} (PE), because the Poisson fluctuation approximates
...
regularization. The offset tells us how much asymmetry
...
\caption{Results of the linearity tests. Artificially induced
...
\item \textbf{Parton distribution functions (PDFs):} The systematic
...
Evaluating most of these systematics requires us to regenerate the
...
evaluate the systematic uncertainty due to limited \textbf{Monte
...
covered by other systematics, we try reweighting both the bias
distribution and $S$ together. The uncertainty produced by this test
is still smaller in magnitude than the sum of the other systematics,
so we can be confident that those systematics cover any mismodeling in
$S$.

Finally, we verify that the regularization term contributes only a
...
events recorded by the CMS experiment at the LHC. The data sample was
...
the unfolding technique to extrapolate our results to parton level. These
...
are consistent with the predictions of the Standard Model, and as a
